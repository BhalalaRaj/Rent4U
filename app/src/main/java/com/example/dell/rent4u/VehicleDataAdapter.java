...
class VehicleDataAdapter extends FirebaseRecyclerAdapter<VehicleDataClass, VehicleDataAdapter.VehicleViewHolder> {
    ...
    public void onBindViewHolder(@NonNull VehicleViewHolder holder, int position, @NonNull List<Object> payloads) {
        ...
        Glide.with(context).load(d.getFrontImage()).into(holder.image);
        holder.vehicleName.setText(d.getVehicleName() + "|" + d.getNumberPlate());
        holder.rentPerKm.setText("Rent" + d.getRentPrice());
        holder.vehicleType.setText("Type: " + d.getVehicleType());
        holder.seatingCapacity.setText("Seating; " + d.getSeatingCapacity());
    }
